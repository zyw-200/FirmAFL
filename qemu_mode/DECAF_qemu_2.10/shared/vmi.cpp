...
{ 
	...
	if(GuestOS_index_c == 0 || GuestOS_index_c == 1)
		monitor_printf(cur_mon, "its win xp \n");
	else if(GuestOS_index_c == 2 || GuestOS_index_c == 3)
		monitor_printf(cur_mon, "its win 7 \n");
	else if(GuestOS_index_c == 4)
		monitor_printf(cur_mon, "its linux \n");
#endif
...
_skip_probe:
	...
	if (count_out-- <= 0)	{// does not find
		DECAF_unregister_callback(DECAF_TLB_EXEC_CB, insn_handle_c);
		monitor_printf(cur_mon, "oops! guest OS type cannot be decided. \n");
	}
}
...
module * VMI_find_module_by_name(const char *name, target_ulong pgd, target_ulong *base)
{
	...
	if(!proc->modules_extracted)
		printf("In VMI_find_module_by_name, current_cpu is %x\n",current_cpu);
		traverse_mmap(current_cpu, proc);
...
{
	monitor_printf(cur_mon,"updating name : not implemented\n");
}
int VMI_remove_all()
{
	monitor_printf(cur_mon,"remove all not implemented\n");
}
...
void VMI_init()
{
	...
	//monitor_printf(cur_mon, "inside vmi init \n");
