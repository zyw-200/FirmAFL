...
void get_executable_directory(string &sPath)
{
  ...
  if(-1 == rval)
  {
    monitor_printf(cur_mon, "can't get path of main executable.\n");
    return;
  }
  szPath[rval-1] = '\0';
  sPath = szPath;
...
int find_match_section(const boost::property_tree::ptree &pt, target_ulong tulInitTaskAddr)
{
    ...
    monitor_printf(cur_mon, "Total Sections: %d\n", cntSection);
    ...
    if(vMatchNum.size() > 1)
    {
      monitor_printf(cur_mon, "Too many match sections in procinfo.ini\n");
      return 0;
    ...
    if(vMatchNum.size() <= 0)
    {
      monitor_printf(cur_mon, "No match in procinfo.ini\n");
      return 0;        
    }
...
//zyw
  ...
  {
      monitor_printf(cur_mon, "can't open %s\n", sProcInfoPath.c_str());
  ...
  monitor_printf(cur_mon, "Match %s\n", pi.strName);
  return 0;
}
...
private:
  ...
  void load()
  {
    ...
    monitor_printf(cur_mon, "Lib Configuration Total Sections: %d\n", cntSection);
  ...
  {
    monitor_printf(cur_mon, "loading lib conf for %s\n", m_cur_libpath.c_str());
