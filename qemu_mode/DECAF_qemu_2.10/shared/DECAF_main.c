...
static gpa_t _DECAF_get_phys_addr(CPUState* env, gva_t addr) {
	...
	if (__builtin_expect(
		...
		if (__builtin_expect(
				env_ptr->tlb_table[mmu_idx][index].addr_code
						!= (addr & TARGET_PAGE_MASK), 0)) {
...
gpa_t DECAF_get_phys_addr(CPUState* env, gva_t addr)
{
	...
	//zyw
	CPUArchState *env_ptr = (CPUArchState *)env->env_ptr;
	uint32_t ori_hflags = env_ptr->hflags;
	env_ptr->hflags &= ~MIPS_HFLAG_UM;
	env_ptr->hflags &= ~MIPS_HFLAG_SM;
...
/*
void DECAF_flushTranslationBlock_env(CPUState *env, target_ulong addr) {
	TranslationBlock *tb;
...
/*
void DECAF_flushTranslationPage_env(CPUState* env, target_ulong addr)
{
	if (env == NULL ) {
#ifdef DECAF_NO_FAIL_SAFE
		return;
...
int do_unload_plugin(Monitor *mon, const QDict *qdict, QObject **ret_data) {
	if (decaf_plugin_path[0]) {
		...
		monitor_printf(cur_mon, "%s is unloaded!\n", decaf_plugin_path);
...
void DECAF_loadvm(void *opaque) {
	...
	if (loadvm_args[0]) {
		//zyw
		load_snapshot(loadvm_args[0], NULL);
	...
	if (loadvm_args[1]) {
		...
		do_load_plugin_internal(cur_mon, loadvm_args[1]);
		free(loadvm_args[1]);
		loadvm_args[1] = NULL;
	}
...
static int DECAF_load(QEMUFile * f, void *opaque, int version_id) {
	...
	if (len > 1)
		...
		do_load_plugin_internal(cur_mon, tmp_plugin_path);
...
#ifdef CONFIG_TCG_TAINT
	//zyw
	CPUArchState *env_ptr = (CPUArchState *)current_cpu->env_ptr;
	taintcheck_nic_writebuf(addr, size, (uint8_t *) &(env_ptr->tempidx));
#endif
}
...
void DECAF_nic_in(const uint32_t addr, const int size) {
	...
	//zyw
	CPUArchState *env_ptr = (CPUArchState *)current_cpu->env_ptr;
	taintcheck_nic_readbuf(addr, size, (uint8_t *) &(env_ptr->tempidx));
#endif
}
/*
...
void DECAF_keystroke_read(uint8_t taint_status) {
	...
	if (taint_keystroke_enabled) {
		//zyw
		CPUArchState *env_ptr = (CPUArchState *)current_cpu->env_ptr;
		env_ptr->tempidx = taint_status;
		env_ptr->tempidx = env_ptr->tempidx & 0xFF;
	}
#endif /*CONFIG_TCG_TAINT*/
}
