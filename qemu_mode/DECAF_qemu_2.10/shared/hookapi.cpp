...
{
        if(current_cpu == NULL) return;

        target_ulong pc = DECAF_getPC(current_cpu);
        target_ulong pgd = DECAF_getPGD(current_cpu);
    ...
    struct hookapi_record_list_head *head =
        ...
        for(record = head->lh_first; record;  record = tmp) {
            ...
            if(record->esp && DECAF_getESP(current_cpu) - record->esp > 80)
                continue;
               ...
               )
{
  ...
  record->esp = DECAF_getESP(current_cpu);
  record->cr3 = DECAF_getPGD(current_cpu);
...
void check_unresolved_hooks()
{
	...
	for(iter=fun_to_hook.begin(); iter!=fun_to_hook.end(); iter++) {
		...
		//zyw
		//monitor_printf(cur_mon, "Hooking %s::%s at 0x%08x\n",
