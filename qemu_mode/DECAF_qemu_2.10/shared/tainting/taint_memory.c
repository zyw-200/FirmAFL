...
void  __taint_ldb_raw_paddr(ram_addr_t addr,gva_t vaddr)
{
...
void  __taint_ldw_raw_paddr(ram_addr_t addr,gva_t vaddr)
{	
	CPUArchState *cpu_single_env = (CPUArchState *)current_cpu->env_ptr;
	unsigned int middle_node_index;
	  unsigned int leaf_node_index;
...
void  __taint_ldl_raw_paddr(ram_addr_t addr,gva_t vaddr)
{
	CPUArchState *cpu_single_env = (CPUArchState *)current_cpu->env_ptr;
	 unsigned int middle_node_index;
	  unsigned int leaf_node_index;
...
void  __taint_ldq_raw_paddr(ram_addr_t addr,gva_t vaddr)
{
	CPUArchState *cpu_single_env = (CPUArchState *)current_cpu->env_ptr;
	  unsigned int middle_node_index;
	  unsigned int leaf_node_index;
...
void  __taint_ldb_raw(unsigned long addr, gva_t vaddr) {
	addr = qemu_ram_addr_from_host_nofail((void*)addr);
	__taint_ldb_raw_paddr(addr,vaddr);

}

void  __taint_ldw_raw(unsigned long addr, gva_t vaddr) {
	addr = qemu_ram_addr_from_host_nofail((void*)addr);
	__taint_ldw_raw_paddr(addr,vaddr);
}

void  __taint_ldl_raw(unsigned long addr, gva_t vaddr) {
	addr = qemu_ram_addr_from_host_nofail((void*)addr);
	__taint_ldl_raw_paddr(addr,vaddr);
}

void  __taint_ldq_raw(unsigned long addr, gva_t vaddr) {
	addr = qemu_ram_addr_from_host_nofail((void*)addr);
	__taint_ldq_raw_paddr(addr,vaddr);
}

void  __taint_stb_raw_paddr(ram_addr_t addr,gva_t vaddr) {
...
void  __taint_stw_raw_paddr(ram_addr_t addr,gva_t vaddr) {
	if (!taint_memory_page_table || addr >= ram_size)
		return;
	CPUArchState *cpu_single_env = (CPUArchState *)current_cpu->env_ptr;
...
void  __taint_stl_raw_paddr(ram_addr_t addr,gva_t vaddr) {
	if (!taint_memory_page_table || addr >= ram_size)
		return;
	CPUArchState *cpu_single_env = (CPUArchState *)current_cpu->env_ptr;
...
void  __taint_stq_raw_paddr(ram_addr_t addr,gva_t vaddr) {
	if (!taint_memory_page_table || addr >= ram_size)
		return;
	CPUArchState *cpu_single_env = (CPUArchState *)current_cpu->env_ptr;
	/* AWH - Keep track of whether the taint state has changed for this location.
...
void  __taint_stb_raw(unsigned long addr, gva_t vaddr) {
	addr = qemu_ram_addr_from_host_nofail((void*)addr);
	__taint_stb_raw_paddr(addr, vaddr);
}

void  __taint_stw_raw(unsigned long addr, gva_t vaddr) {
	addr = qemu_ram_addr_from_host_nofail((void*)addr);
	__taint_stw_raw_paddr(addr, vaddr);
}

void  __taint_stl_raw(unsigned long addr, gva_t vaddr) {
	addr = qemu_ram_addr_from_host_nofail((void*)addr);
	__taint_stl_raw_paddr(addr, vaddr);
}

void  __taint_stq_raw(unsigned long addr, gva_t vaddr) {
	addr = qemu_ram_addr_from_host_nofail((void*)addr);
...
void do_enable_tainting_internal(void) {
  if (!taint_tracking_enabled) {
    CPUState *env = current_cpu ? current_cpu : first_cpu;
    DECAF_stop_vm();
    tb_flush(env);
...
int do_enable_tainting(Monitor *mon, const QDict *qdict, QObject **ret_data) {
  if (!taint_tracking_enabled) {
    ...
    monitor_printf(cur_mon,  "Taint tracking is now enabled (fresh taint data generated)\n");
  } else
    monitor_printf(cur_mon, "Taint tracking is already enabled\n");
  return 0;
}
...
int do_disable_tainting(Monitor *mon, const QDict *qdict, QObject **ret_data) {
  if (taint_tracking_enabled) {
    ...
    monitor_printf(cur_mon, "Taint tracking is now disabled (all taint data discarded)\n");
  } else
    monitor_printf(cur_mon, "Taint tracking is already disabled\n");
  return 0;
}
...
int do_taint_nic_off(Monitor *mon, const QDict *qdict, QObject **ret_data) {
  if (taint_tracking_enabled && taint_nic_enabled) {
    ...
    monitor_printf(cur_mon, "NIC tainting is now disabled.\n");
  } else if (taint_tracking_enabled) {
    monitor_printf(cur_mon, "Ignored, NIC tainting was already disabled.\n");
  } else
    monitor_printf(cur_mon, "Ignored, taint tracking is disabled.  Use the 'enable_tainting' command to enable tainting first.\n");
  return 0;
}
...
int do_taint_nic_on(Monitor *mon, const QDict *qdict, QObject **ret_data) {
  if (taint_tracking_enabled && !taint_nic_enabled) {
    ...
    monitor_printf(cur_mon, "NIC tainting is now enabled.\n");
  } else if (taint_tracking_enabled) {
    monitor_printf(cur_mon, "Ignored, NIC tainting was already enabled.\n");
  } else
    monitor_printf(cur_mon, "Ignored, taint tracking is disabled.  Use the 'enable_tainting' command to enable tainting first.\n");
  return 0;
}
int do_tainted_bytes(Monitor *mon,const QDict *qdict,QObject **ret_data){
  ...
  if(!taint_tracking_enabled)
    monitor_printf(cur_mon,"Taint tracking is disabled,no statistics available\n"); 
  else{
     tainted_bytes=calc_tainted_bytes();
     monitor_printf(cur_mon,"Tainted memory: %d bytes\n",tainted_bytes);
  }
  return 0;
}
int do_taint_mem_usage(Monitor *mon, const QDict *qdict, QObject **ret_data) {
  if (!taint_tracking_enabled)
    ...
    monitor_printf(cur_mon, "%uM RAM: %d mid nodes, %d leaf nodes, %d/%d mid pool, %d/%d leaf pool\n",
...
int do_garbage_collect_taint(Monitor *mon, const QDict *qdict, QObject **ret_data) {
  if (!taint_tracking_enabled)
    monitor_printf(cur_mon, "Ignored, taint tracking is disabled\n");
  else
  {
    ...
    monitor_printf(cur_mon, "Garbage Collector: Removed %d mid nodes, %d leaf nodes\n", prior_middle - middle_nodes_in_use, prior_leaf - leaf_nodes_in_use);
  }
  return 0;
