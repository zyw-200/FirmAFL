...
static inline uint64_t taintcheck_register_check(int regid,int offset,int size,CPUState *env){
    ...
    return (size < 4) ? (env_ptr->active_tc.taint_gpr[regid]>>off)
        &size_to_mask(size):env_ptr->active_tc.taint_gpr[regid]>>off;
#else
    return (size < 4) ? (env_ptr->taint_regs[regid]>>off)&size_to_mask(size):
    		env_ptr->taint_regs[regid]>>off;
#endif /* CONFIG_MIPS */
}
/*
addr: physical addr
