...
int DECAF_get_page_access(CPUState* env, /*uint32_t*/target_ulong addr)
{
    ...
    CPUArchState * env_ptr = (CPUArchState *) env->env_ptr;
    if (env_ptr->cr[4] & CR4_PAE_MASK) {
	uint32_t pdpe_addr, pde_addr, pte_addr;
	uint32_t pdpe;

	pdpe_addr = ((env_ptr->cr[3] & ~0x1f) + ((addr >> 30) << 3)) &
	    env_ptr->a20_mask;
	pdpe = ldl_phys(env->as, pdpe_addr);
    ...
    pde_addr =
	((env_ptr->cr[3] & ~0xfff) + ((addr >> 20) & ~3)) & env_ptr->a20_mask;
    pde = ldl_phys(env->as, pde_addr);
    if (!(pde & PG_PRESENT_MASK))
	return -1;
    if ((pde & PG_PSE_MASK) && (env_ptr->cr[4] & CR4_PSE_MASK)) {
	/* page size is 4MB */
	return (pde & PG_RW_MASK);
    }

    /* page directory entry */
    pte_addr = ((pde & ~0xfff) + ((addr >> 10) & 0xffc)) & env_ptr->a20_mask;
    pte = ldl_phys(env->as, pte_addr);
    if (!(pte & PG_PRESENT_MASK))
	return -1;
