...
#include "config-host.h" // zyw
#include "config-target.h" // zyw
#include <assert.h>
...
static inline int DECAF_is_in_kernel(CPUState *env)
{
  ...
  return ((env_ptr->hflags & MIPS_HFLAG_MODE) == MIPS_HFLAG_KM);
...
static inline gva_t DECAF_getESP(CPUState* env)
{
  ...
  // zyw
  CPUArchState *env_ptr = (CPUArchState *)env->env_ptr;
  return (env_ptr->active_tc.gpr[29]);
