...
#include "shared/DECAF_callback_to_QEMU.h" // zyw
...
target_ulong defined_helper_aflCall(CPUArchState *env, target_ulong code, target_ulong a0, target_ulong a1);
...
/*
    case OPC_LDL:
 	env->active_tc.gpr[2] = defined_helper_aflCall(env, env->active_tc.gpr[4], env->active_tc.gpr[5], env->active_tc.gpr[6]);// first argument cpu_env changed to env	
        break;
*/
#if defined(TARGET_MIPS64)
    /* MIPS64 opcodes */
...
void gen_intermediate_code(CPUState *cs, struct TranslationBlock *tb)
{
    ...
    if(DECAF_is_BlockBeginCallback_needed(tb->pc))
    {
      ...
      // which implies that the TCG target is the HOSTz
      TCGv_ptr tmpTb = tcg_const_ptr((tcg_target_ulong)tb);
...
#if 1 /* AWH */
        //if(DECAF_is_callback_needed(DECAF_INSN_END_CB)) { gen_helper_DECAF_invoke_insn_end_callback(cpu_env); }
	if(DECAF_is_callback_needed(DECAF_INSN_END_CB)) {
		TCGv pc = tcg_temp_new();
		tcg_gen_movi_tl(pc, ctx.pc);
		gen_helper_DECAF_invoke_insn_end_callback(cpu_env, pc); 
	}
#endif /* AWH */
